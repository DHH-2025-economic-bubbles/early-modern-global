...
# Define file paths
graph_file = DATA_FOLDER / "articles_west_indies/articles_west_indies_with_persons.jsonl"
clustering_file = DATA_FOLDER / "articles_west_indies/clustering_west_indies.jsonl"
cluster_names_file = DATA_FOLDER / "articles_west_indies/clustering.csv"
output_visualization = DATA_FOLDER / "articles_west_indies/clustering_west_indies_network_visualization.png"
...
def apply_community_based_layout(G, node_to_community, iterations=10000):
    """
    ...
    forceatlas2 = ForceAtlas2(
        ...
        scalingRatio=20,                     # INCREASED: Stronger repulsion pushes communities apart
        strongGravityMode=True,              # Let communities form naturally
        gravity=1.0,                          # REDUCED: Less gravity allows communities to spread out
...
def visualize_graph_with_communities():
    ...
    # Draw nodes with community -1 (no community assigned) in gray
    unassigned_nodes = [node for node in G.nodes() if G.nodes[node]['community'] == -1]
    if unassigned_nodes:
        node_size = [20 + G.nodes[node]['weighted_degree'] / 10 for node in unassigned_nodes]
        nx.draw_networkx_nodes(G, pos,
                              nodelist=unassigned_nodes,
                              node_size=node_size,
                              node_color='lightgray',  # Use gray color for unassigned nodes
                              alpha=0.6,  # Slightly more transparent
                              )
    
    # Draw node labels for high-degree nodes only
    degree_threshold = max([G.nodes[node]['degree'] for node in G.nodes()]) / 1.5
    ...
    # Draw community labels - SEPARATE from node drawing
    for comm_id, nodes in communities.items():
        community_nodes = [node for node in nodes if node in G.nodes()]
        if not community_nodes:
            continue

        # Only draw label if cluster name exists
        if comm_id in cluster_names:
            ...
            # Find the outermost node to place label outside the cluster
            max_distance = 0
            for node in community_nodes:
                distance = ((pos[node][0] - centroid_x)**2 + (pos[node][1] - centroid_y)**2)**0.5
                if distance > max_distance:
                    max_distance = distance
            
            # Place label outside the community boundary
            offset = max_distance * 0.3  # Adjust this multiplier as needed
            label_x = centroid_x
            label_y = centroid_y + offset  # Place above the community
            
            # Get cluster name
            cluster_name = f"{comm_id}: {cluster_names[comm_id]}"
            
            # Add a background box for better visibility
            plt.text(label_x, label_y, cluster_name, 
                    fontsize=12, fontweight='bold', 
                    ha='center', va='center',
                    bbox=dict(facecolor='white', alpha=0.9, edgecolor=community_colors[comm_id], 
